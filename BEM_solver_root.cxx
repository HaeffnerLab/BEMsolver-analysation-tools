...
//#include "TGraph2D.h"
#ifdef __CINT__
int rod_trap3()  //put here the name of the file
...
int main(int argc, char* argv[]){
	...
	const int NUM_ELECTRODES=25; //specify number of non-ground electrodes

	D3ImportedElectrodes *impel=new D3ImportedElectrodes();
	TString importfilename;
	logfile.AbsoluteFileName("./D_wire-50.dxf",importfilename);
	if(!impel->Import(importfilename)) return 0;

	ofstream datafile;
	ofstream voltagefile;
	datafile.open ("./D_wire-50_99-129_WR64height_field1.txt"); //select file name for data output
	voltagefile.open("D_wire-50_99-129_WR64heightvoltages1.txt");
	...
	wr->refine(65); //sets the resolution
	...
	for(int curEl = 0; curEl < NUM_ELECTRODES; curEl++){ 
		...
		double xstart=-28; //defines boundaries
		double xstop=2; 
		//center at -30
