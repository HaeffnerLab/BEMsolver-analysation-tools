function imagesc_plot_1d(superData,superEl,outpath,newfilename,myParams)
    ...
    for i=(1:size(movePosVec,2))
        ...
        plot(actPos,plotUnew((movePos+14)*10)+0.1,'o','MarkerFaceColor','r','MarkerSize',5,'MarkerEdgeColor','none')
        plot(myParams.ionTwoPos,plotUnew((nSteps-14)*10)+0.1,'o','MarkerFaceColor','r','MarkerSize',5,'MarkerEdgeColor','none')
    ...
    print(h,'-dpdf',[sprintf('%s%s/',outpath,newfilename) 'imagesc_potential_traj_1d_multi.pdf'])
    saveas(h,[sprintf('%s%s/fig/',outpath,newfilename) 'imagesc_potential_traj_1d_multi.fig'])
